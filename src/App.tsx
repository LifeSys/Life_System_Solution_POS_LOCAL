...
function Admin({ user, setNotice }: { user: UserSession; setNotice: (n: Notice) => void }) {
  ...
  return <div className="space-y-4"><div className="flex gap-2 rounded-xl bg-white p-2 shadow-sm"><button onClick={() => setTab('productos')} className={tab === 'productos' ? 'btn-primary' : 'btn-muted'}>Productos</button><button onClick={() => setTab('usuarios')} className={tab === 'usuarios' ? 'btn-primary' : 'btn-muted'}>Usuarios</button><button onClick={() => setTab('mesas')} className={tab === 'mesas' ? 'btn-primary' : 'btn-muted'}>Mesas</button></div>{tab === 'productos' && <Products user={user} setNotice={setNotice} />}{tab === 'usuarios' && <Users user={user} setNotice={setNotice} />}{tab === 'mesas' && <Panel title="Mesas"><p>Administración single-store de mesas operativas. Se conserva la numeración local sin selector de locales.</p><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{Array.from({ length: 12 }, (_, i) => <div key={i} className="rounded-xl border border-slate-200 bg-white p-4"><b>Mesa {i + 1}</b><p className="text-sm text-slate-500">Capacidad editable pendiente de persistencia.</p></div>)}</div></Panel>}</div>;
}
...
function Sales({ user, setNotice }: { user: UserSession; setNotice: (n: Notice) => void }) {
  ...
  const [cart, setCart] = useState<Array<{ variantId: string; label: string; cantidad: number; precio: number; stock: number }>>([]);
  const [mesa, setMesa] = useState('');
  const [note, setNote] = useState('');
  useEffect(() => { api.products.list().then((r) => { if (r.ok) setProducts(r.data); }); }, []);
  const variants = products.flatMap((p) => p.variants.map((v) => ({ ...v, label: `${p.nombre} ${v.nombre}` })));
  function cartQty(id: string) { return cart.filter((i) => i.variantId === id).reduce((sum, i) => sum + i.cantidad, 0); }
  async function add(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const f = new FormData(event.currentTarget); const v = variants.find((x) => x.id === String(f.get('variantId'))); const cantidad = Number(f.get('cantidad'));
    if (!v || !Number.isInteger(cantidad) || cantidad <= 0) return setNotice({ kind: 'error', text: 'La cantidad debe ser mayor a cero' });
    if (cartQty(v.id) + cantidad > v.stock) return setNotice({ kind: 'error', text: 'Stock insuficiente para agregar al carrito' });
    setCart((prev) => [...prev, { variantId: v.id, label: v.label, cantidad, precio: Number(v.precio), stock: v.stock }]);
  }
  async function confirm() {
    const current = await api.cash.current();
    if (!current.ok || !current.data) return setNotice({ kind: 'error', text: 'Debe abrir una caja antes de registrar una venta.' });
    const r = await api.orders.create({ userId: user.id, mesa, note, items: cart.map((i) => ({ variantId: i.variantId, cantidad: i.cantidad })) });
    if (r.ok) { setCart([]); setMesa(''); setNote(''); setNotice({ kind: 'ok', text: `Pedido ${r.data.id.slice(-6)} creado correctamente` }); } else setNotice({ kind: 'error', text: r.error });
  }
  const total = cart.reduce((sum, item) => sum + item.precio * item.cantidad, 0);
  return <Panel title="Mesas y toma rápida de pedidos"><div className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]"><div><form onSubmit={add} className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 md:grid-cols-[1fr_130px_180px]"><select name="variantId" className="input input-light">{variants.map((v) => <option disabled={v.stock <= 0} key={v.id} value={v.id}>{v.label} · {money(v.precio)} · stock {v.stock}</option>)}</select><input name="cantidad" className="input input-light" type="number" min="1" defaultValue="1" /><button className="btn-primary btn-touch">Agregar</button></form><div className="mt-4 grid gap-3 sm:grid-cols-2">{cart.map((i, idx) => <div key={`${i.variantId}-${idx}`} className="ticket-card"><p className="font-bold text-slate-950">{i.label}</p><p className="text-sm text-slate-500">{money(i.precio)} · subtotal {money(i.precio * i.cantidad)}</p><div className="mt-3 flex items-center justify-between gap-2"><InventoryAdjust onAdjust={(d) => setCart((prev) => prev.flatMap((x, j) => j === idx ? (x.cantidad + d <= 0 ? [] : x.cantidad + d > x.stock ? [x] : [{ ...x, cantidad: x.cantidad + d }]) : [x]))} /><button onClick={() => setCart(cart.filter((_, j) => j !== idx))} className="btn-danger">Eliminar</button></div></div>)}{!cart.length && <p className="rounded-2xl border border-dashed border-slate-300 bg-white p-6 text-slate-500">Agrega productos reales del catálogo para crear un pedido.</p>}</div></div><aside className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><h3 className="text-lg font-bold">Datos del ticket</h3><label className="label mt-4">Mesa<input value={mesa} onChange={(e) => setMesa(e.target.value)} className="input input-light mt-1" placeholder="Ej. Mesa 4" /></label><label className="label mt-3">Observaciones<textarea value={note} onChange={(e) => setNote(e.target.value)} className="input input-light mt-1 min-h-24" placeholder="Sin cebolla, para llevar..." /></label><div className="mt-5 flex items-center justify-between border-t border-slate-200 pt-4"><b className="text-xl">{money(total)}</b><button disabled={!cart.length} onClick={confirm} className="btn-primary btn-touch disabled:opacity-50">Confirmar</button></div></aside></div></Panel>;
}
