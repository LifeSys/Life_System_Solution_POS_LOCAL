...
const api: Api = {
  config: { exists: () => invoke('config:exists'), save: (config) => invoke('config:save', config), startupState: () => invoke('config:startupState') },
  auth: { login: (request) => invoke('auth:login', request), logout: () => invoke('auth:logout'), createInitialAdmin: (request) => invoke('auth:createInitialAdmin', request) },
