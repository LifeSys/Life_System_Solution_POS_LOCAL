...
function createWindow() {
  const win = new BrowserWindow({ width: 1200, height: 800, webPreferences: { preload: join(__dirname, 'preload.js'), contextIsolation: true, nodeIntegration: false, sandbox: true } });
...
app.whenReady().then(() => {
  registerConfigIpc(); registerAuthIpc(); registerOrdersIpc(); registerInventoryIpc(); registerCashIpc(); registerProductsIpc();
  if (hasDbConfig()) runMigrations();
  createWindow();
});
