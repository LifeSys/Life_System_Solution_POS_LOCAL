...
import type { CreateOrderRequest, UpdateOrderStatusRequest } from '../../shared/ipc.js';
...
export function registerOrdersIpc() {
  ...
  ipcMain.handle('orders:updateStatus', (_e, req: UpdateOrderStatusRequest) => wrap(async () => getPrisma().$transaction(async (tx) => {
    ...
    if (req.estado !== 'PAGADO') return tx.order.update({ where: { id: req.orderId }, data: { estado: req.estado } });
  ...
  ipcMain.handle('orders:getById', (_e, id: string) => wrap(() => getPrisma().order.findUnique({ where: { id }, include: { items: true } })));
}
