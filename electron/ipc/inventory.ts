...
const mapInventory = (inventory: any) => ({ variantId: inventory.variant_id, productId: inventory.variant.product.id, producto: inventory.variant.product.nombre, variante: inventory.variant.nombre, precio: toMoney(inventory.variant.precio), stock: inventory.current_stock });
...
export function registerInventoryIpc() {
  ...
  ipcMain.handle('inventory:adjust', (_e, req: InventoryAdjustRequest) => wrap(() => getPrisma().$transaction(async (tx) => {
    ...
    await tx.auditLog.create({ data: { user_id: req.userId, accion: 'INVENTORY_ADJUSTED', detalle_json: safeDetail({ variantId: req.variantId, delta: req.delta, reason: req.reason }) } });
